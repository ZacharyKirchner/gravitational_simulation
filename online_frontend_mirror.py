...
app.layout = html.Div(children=[
    ...
    dbc.Collapse(dash_table.DataTable(id="input_table",columns=[{'name':standard_columns[n],'id':n} for n in range(len(standard_columns))],data=[{"Name":"bleh","x":"splango"},{"Name":"foo","y":"bar"}],editable=True),id='collapse',is_open=True),
    dbc.Button('Toggle table visibility',id='collapse_button',n_clicks=0),
    ...
    dbc.Input(id="timestep",type='number',min=1,value=1),
    ...
    dbc.Input(id="duration",type='number',min=1,value=1,max=100000000),
    #html.Span('Days')]),
    dbc.Button('Start',id="run_button",n_clicks=0),
...
def add_row(n_clicks,rows,columns):
    ...
    return rows'''
@app.callback(dash.dependencies.Output('collapse','is_open'),[dash.dependencies.Input('collapse_button','n_clicks')],[dash.dependencies.State('collapse','is_open')])
